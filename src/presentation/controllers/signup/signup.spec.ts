...
const makeFakeRequest = ({ name, password, passwordConfirmation, email }: any): HttpRequest => {
  return {
    body: {
      name: name ?? 'any_name',
      password: password ?? 'any_password',
      passwordConfirmation: passwordConfirmation ?? 'any_password',
...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if password confirmation fails', async () => {
    const { sut } = makeSut()
    const httpRequest = makeFakeRequest({ passwordConfirmation: 'invalid_password_confirmation' })
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
    expect(httpResponse.body).toEqual(
